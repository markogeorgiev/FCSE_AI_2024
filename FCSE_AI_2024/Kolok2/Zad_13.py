...
def get_first_split(coeff):
    return zero_set[:get_split_index(zero_set, coeff)] + \
        one_set[:get_split_index(one_set, coeff)] + \
        two_set[:get_split_index(two_set, coeff)]


def get_middle_split(l_coeff, r_coeff):
    return zero_set[get_split_index(zero_set, l_coeff):get_split_index(zero_set, r_coeff)] + \
        one_set[get_split_index(one_set, l_coeff):get_split_index(one_set, r_coeff)] + \
        two_set[get_split_index(two_set, l_coeff):get_split_index(two_set, r_coeff)]


def get_last_split(coeff):
    return zero_set[get_split_index(zero_set, coeff):] + \
        one_set[get_split_index(one_set, coeff):] + \
        two_set[get_split_index(two_set, coeff):]


def get_accuracy(preds1, preds2, preds3, true):
    counter = 0
    for p1, p2, p3, t in zip(preds1, preds2, preds3, true):
        curr = 0
        if p1 == t: curr += 1
        if p2 == t: curr += 1
        if p3 == t: curr += 1
...
if __name__ == '__main__':
    ...
    test_set = get_last_split(x2)
    test_x, test_y = get_tx_ty(test_set)

    train_set_1 = get_first_split(x1)
    train_x_1, train_y_1 = get_tx_ty(train_set_1)

    train_set_2 = get_middle_split(x1, x2)
    train_x_2, train_y_2 = get_tx_ty(train_set_2)

    train_set_3 = get_first_split(x2)
    train_x_3, train_y_3 = get_tx_ty(train_set_3)

    clf1 = GaussianNB()
    clf2 = DecisionTreeClassifier(random_state=0)
    clf3 = RandomForestClassifier(random_state=0, n_estimators=3)

    clf1.fit(train_x_1, train_y_1)
    clf2.fit(train_x_2, train_y_2)
    clf3.fit(train_x_3, train_y_3)

    pred1 = clf1.predict(test_x)
    pred2 = clf2.predict(test_x)
    pred3 = clf3.predict(test_x)

    print(f'Tochnost: {get_accuracy(pred1, pred2, pred3, test_y)}')