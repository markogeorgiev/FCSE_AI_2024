...
if __name__ == '__main__':
    ...
    crit = 'gini'
    clf_decision_tree = DecisionTreeClassifier(criterion=crit, random_state=0)
    ...
    importeaces = list(clf_decision_tree.feature_importances_)
    index_min = importeaces.index(min(importeaces))
    dataset = nu.delete(dataset, index_min, 1)

    train_set = nu.delete(train_set, index_min, 1)
    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]
    ...
    clf_min_max_mlp = MLPClassifier(n_hidden_layers, activation=a_funct, learning_rate_init=l_rate, max_iter=n_epochs, random_state=0)
    clf_standard_mlp = MLPClassifier(n_hidden_layers, activation=a_funct, learning_rate_init=l_rate, max_iter=n_epochs, random_state=0)

    scaler_min_max = MinMaxScaler()
    scaler_standard = StandardScaler()

    scaler_min_max.fit(train_x)
    scaler_standard.fit(train_x)

    train_x_min_max = scaler_min_max.transform(train_x)
    train_x_standard = scaler_standard.transform(train_x)

    test_x_min_max = scaler_min_max.transform(test_x)
    test_x_standard = scaler_standard.transform(test_x)

    clf_min_max_mlp.fit(train_x_min_max, train_y)
    clf_standard_mlp.fit(train_x_standard, train_y)

    print(f'Tocnost so StandardScaler: {clf_standard_mlp.score(test_x_standard, test_y)}')
    print(f'Tocnost so MinMaxScaler: {clf_min_max_mlp.score(test_x_min_max, test_y)}')
