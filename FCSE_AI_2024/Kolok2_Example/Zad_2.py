...
def get_split_index(dset, x):
    return int(len(dset) * x)


def get_tr_te(o_set, z_set, x):
    tr = o_set[:get_split_index(o_set, x)] + z_set[:get_split_index(z_set, x)]
    te = o_set[get_split_index(o_set, x):] + z_set[get_split_index(z_set, x):]
    return tr, te
...
def get_o_z(dset):
    o_set = [row for row in dset if row[-1] == 1]
    z_set = [row for row in dset if row[-1] == 0]
    return o_set, z_set
...
if __name__ == '__main__':
    ...
    one_set, zero_set = get_o_z(dataset)

    train_set, test_set = get_tr_te(one_set, zero_set, x)
    train_x, train_y = get_tx_ty(train_set)
    test_x, test_y = get_tx_ty(test_set)

    clf_nb = GaussianNB()
    clf_dt = DecisionTreeClassifier(criterion='entropy', random_state=0)
    clf_fr = RandomForestClassifier(n_estimators=4, criterion='entropy', random_state=0)
    clf_ml = MLPClassifier(10, activation='relu', learning_rate_init=0.001, random_state=0)

    clf_dict = {'Naive Bayes': clf_nb,
                'Decision Tree': clf_dt,
                'Random Forest': clf_fr,
                'MLP': clf_ml}
    acc = dict()
    ...
    for clf, clf_n in zip(clf_dict.values(), clf_dict.keys()):
        clf.fit(train_x, train_y)
        acc[clf_n] = clf.score(test_x, test_y)
        if max_acc < acc[clf_n]:
            max_acc = acc[clf_n]
            max_name = clf_n
            max_clf = clf

    print(f'Najgolema tocnost ima klasifikatorot {max_name}')
    clf_weights = dict()
    clf_weights[max_clf] = 2

    for clf, clf_n in zip(clf_dict.values(), clf_dict.keys()):
        if clf_n != max_name:
            clf_weights[clf] = 1

    points = [[0, 0] for _ in range(len(test_y))]

    for clf, clf_w in zip(clf_weights.keys(), clf_weights.values()):
        for i in range(len(test_x)):
            prediction = clf.predict([test_x[i]])[0]
            if prediction == 0:
                points[i][0] += clf_w
            elif prediction == 1:
                points[i][1] += clf_w
    def get_prediction(c_points):
    ...
    for i in range(len(test_y)):
        prediction = get_prediction(points[i])
        temp.append(prediction)
        if test_y[i] == 1 and prediction == 1:
            tp += 1
        elif test_y[i] == 1 and prediction == 0:
            fn += 1
