...
class AllColors(Problem):
    def __init__(self, initial, goal_state, n):
    ...
    def successor(self, state):
        ...
        for i in range(n):
            for j in range(n):
                successors[f'x: {i}, y: {j}'] = self.make_new_state(i, j, copy.deepcopy(state))

        return successors

    def make_new_state(self, i, j, state):
        new_state = make_lists(state)
        new_state[i][j] = abs(state[i][j] - 1)

        # i + 1
        if i + 1 < self.n:
            new_state[i + 1][j] = abs(state[i + 1][j] - 1)
            # i - 1
        if i - 1 >= 0:
            new_state[i - 1][j] = abs(state[i - 1][j] - 1)
        # j + 1
        if j + 1 < self.n:
            new_state[i][j + 1] = abs(state[i][j + 1] - 1)
        # j - 1
        if j - 1 >= 0:
            new_state[i][j - 1] = abs(state[i][j - 1] - 1)

        new_state = make_hashable(new_state)
...
if __name__ == "__main__":
    ...
    goal_state = [[1] * n for _ in range(n)]

    problem = AllColors(make_hashable(matrix), make_hashable(goal_state), n)
