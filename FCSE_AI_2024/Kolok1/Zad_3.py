...
def make_everything_tuple(to_convert):
    return tuple(tuple(sublist) for sublist in to_convert)

def make_everything_list(to_convert):
    return [list(subtuple) for subtuple in to_convert]
        ...
        for i in range(len(state)):
            if len(state[i]) != 0:
                for j in range(len(state)):
                    if j != i:
                        new_state = self.make_state(i, j, state)
                        if new_state is not None:
                            successors[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = new_state
        return successors

    def make_state(self, from_index, to_index, state):
        full_list = make_everything_list(state)
        if not full_list[from_index]:
            return None

        to_move = full_list[from_index].pop()

        if not full_list[to_index] or to_move <= full_list[to_index][-1]:
            full_list[to_index].append(to_move)
            return make_everything_tuple(full_list)
...
if __name__ == "__main__":
    # 3 pillars means 3 arrays which should be kept in some sort of tuple
    inputData = input().split(';')
    startState = []
    for inp in inputData:
        if len(inp) == 0:
            startState.append(list())
        else:
            startState.append(inp.split(','))
    startState = make_everything_tuple(startState)

    inputData = input().split(';')
    goalState = []
    for inp in inputData:
        if len(inp) == 0:
            goalState.append(list())
        else:
            goalState.append(inp.split(','))
    goalState = make_everything_tuple(goalState)

    #print(startState)
    #print(goalState)

    problem = Towers(startState, goalState)
    sol = uniform_cost_search(problem)

    if sol is not None:
        print(f'Number of action {len(sol.solution())}')
        print(sol.solution())
    else:
        print('No solution!')