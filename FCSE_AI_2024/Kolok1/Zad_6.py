...
class Squares(Problem):
    ...
    @staticmethod
    def check_valid(new_tile):
        return 0 <= new_tile[0] < 5 and 0 < new_tile[1] <= 5

    def make_new_state(self, new_tile, at_index, all_tiles):
        all_tiles = list(all_tiles)
        all_tiles[at_index] = new_tile
        return tuple(all_tiles)
    ...
    def successor(self, state):
        ...
        for index in range(5):
            curr_tile_x, curr_tile_y = state[index]
            # up
            new_tile = curr_tile_x, curr_tile_y + 1
            if self.check_valid(new_tile):
                successors['Pomesti kvadratche ' + (index + 1).__str__() + ' gore'] = self.make_new_state(new_tile, index, state)
            # down
            new_tile = curr_tile_x, curr_tile_y - 1
            if self.check_valid(new_tile):
                successors['Pomesti kvadratche ' + (index + 1).__str__() + ' dolu'] = self.make_new_state(new_tile, index, state)
            # left
            new_tile = curr_tile_x - 1, curr_tile_y
            if self.check_valid(new_tile):
                successors['Pomesti kvadratche ' + (index + 1).__str__() + ' levo'] = self.make_new_state(new_tile, index, state)
            # right
            new_tile = curr_tile_x + 1, curr_tile_y
            if self.check_valid(new_tile):
                successors['Pomesti kvadratche ' + (index + 1).__str__() + ' desno'] = self.make_new_state(new_tile, index, state)
        return successors
    ...
    def h(self, node):
        ...
        for index in range(5):
            curr_state = node.state[index]
            relevant_goal_state = goal_state[index]
            total_manhattan_distance += max(abs(curr_state[0] - relevant_goal_state[0]) + abs(curr_state[1] - relevant_goal_state[1]),
...
if __name__ == '__main__':
    ...
    sol = breadth_first_graph_search(squares)
