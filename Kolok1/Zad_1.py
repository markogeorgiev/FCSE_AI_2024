...
class GhostOnSkates(Problem):
    def __init__(self, initial, holes, grid_size, goal):
    ...
    def ghost_fits(self, player):
        return player not in self.holes \
            and 0 <= player[0] < self.grid_size \
            and 0 <= player[1] < self.grid_size
    ...
    def successor(self, state):
        ...
        #up-1
        new_ghost = ghost_x, ghost_y + 1
        if self.ghost_fits(new_ghost):
            successors['Gore 1'] = new_ghost
        #up-2
        new_ghost = ghost_x, ghost_y + 2
        if self.ghost_fits(new_ghost):
            successors['Gore 2'] = new_ghost
        #up-3
        new_ghost = ghost_x, ghost_y + 3
        if self.ghost_fits(new_ghost):
            successors['Gore 3'] = new_ghost
        #right-1
        new_ghost = ghost_x + 1, ghost_y
        if self.ghost_fits(new_ghost):
            successors['Desno 1'] = new_ghost
        #right-2
        new_ghost = ghost_x + 2, ghost_y
        if self.ghost_fits(new_ghost):
            successors['Desno 2'] = new_ghost
        #right-3
        new_ghost = ghost_x + 3, ghost_y
        if self.ghost_fits(new_ghost):
...
if __name__ == '__main__':
    ...
    inital = ghost_pos
    pacman = GhostOnSkates(inital, holes, n, goal_pos)

    sol = astar_search(pacman)
